/*
 * ===================================================================
 *   QUAN LY THU CHI CA NHAN - PHIEN BAN TONG HOP SIEU CAP
 *   Tong hop uu diem tu ducanh.cpp va QuanLyChiTieu.cpp
 *   Ngon ngu: C++ (chuan C++11)
 *   Compile : g++ -std=c++11 -Wall -o qltc KetHop_final.cpp
 * ===================================================================
 *
 *  UU DIEM DUOC LAY TU TUNG FILE:
 *  [ducanh.cpp]
 *    - He thong quan ly danh muc day du (them / sua / xoa / luu file)
 *    - Ham validation ngay thang chinh xac (nam nhuan, so ngay theo thang)
 *    - formatMoney: in so tien co dau phan cach nghin dep
 *    - inputInt / inputMoney: vong lap nhap lieu an toan, khong crash
 *    - Thong ke theo nam (biet bao nhieu thang co du lieu)
 *    - Bieu do chi tieu text co phan tram va thanh bar #
 *    - Budget: xem, dat/sua, kiem tra trang thai, bao cao vuot han muc
 *    - getMaxID: sinh ID tang dan chinh xac (khong bi trung sau khi xoa)
 *
 *  [QuanLyChiTieu.cpp]
 *    - DynamicArray<T>: mang dong tu viet, tu dong resize (khong gioi han)
 *    - try-catch khi parse file: dong bi hong se bi bo qua, khong crash
 *    - Canh bao REAL-TIME ngay khi nhap giao dich co the vuot budget
 *    - Luu file NGAY SAU MOI thao tac (tranh mat du lieu khi tat dot ngot)
 *    - BudgetArray::update_or_insert: cap nhat hoac them moi thong minh
 *    - ensure_all_data_loaded: dong bo du lieu khi khoi dong
 *
 *  TINH NANG MOI / BUG FIX:
 *    - Xoa giao dich co xac nhan va hien thi thong tin truoc khi xoa
 *    - Tim kiem khong phan biet hoa/thuong (case-insensitive)
 *    - Loc giao dich theo ngay cu the, theo thang, theo danh muc
 *    - Thong ke co tong cuoi bang (thu, chi, can doi)
 *    - Ghi chep ngay giao dich duoc validate truoc khi luu
 *    - Khong dung thu vien ngoai (string, iostream, fstream la C++ standard)
 * ===================================================================
 */
...
#include <cstdio>    // remove(), rename() - can thiet cho DevC++
#include <cstdlib>   // exit()
#include <cstring>   // strcmp (du phong)

using namespace std;

// ===================================================================
// HANG SO TOAN CUC
// Su dung constexpr/const thay the #define (C++ thuan)
// ===================================================================
...
struct Category {
    int    id;
    string name;
    bool   active;   // true = dang dung, false = da "xoa mem"
};
...
// ===================================================================
// MODULE 2: MANG DONG TU VIET (DynamicArray<T>)
// Lay tu QuanLyChiTieu.cpp, bo sung ham sua theo chi so
// ===================================================================
...
public:
    ...
    // Tim vi tri phan tu theo dieu kien (tra ve -1 neu khong co)
...
// ===================================================================
// MODULE 4: TIEN ICH (UTILITY)
// ===================================================================

// Xoa ky tu thua trong bo dem stdin (dung C++ thuan)
void clearInput() {
    cin.ignore(10000, '\n');
}

// In so tien co dau '.' phan cach nghin: 1234567 -> "1.234.567"
...
// Tra ve so ngay trong thang (co tinh nam nhuan)
...
// Ham tim kiem chuoi con (case-insensitive, thuat toan truot cua so)
...
// Trim dau va cuoi chuoi (xu ly CRLF khi doc file)
...
// Tach chuoi CSV theo dau phay (xu ly dung muc ghi chu co the chua nhieu token)
...
void splitCSV(const string& line, string tokens[], int& count, int maxTok) {
    ...
    // Ghi token cuoi (chi khi con cho)
    if (count < maxTok) {
        tokens[count++] = cur;
    }
}

// Xoa ky tu dau phay trong chuoi (tranh lam hong file CSV)
...
// Kiem tra ten danh muc da ton tai chua (case-insensitive, loai tru ID)
...
// Lay ten danh muc theo ID (tra ve "(Da xoa)" neu khong con hieu luc)
...
// Tim chi so trong gCategories theo ID (tra ve -1 neu khong co)
...
bool saveCategories() {
    ...
    if (remove(CAT_FILE.c_str()) != 0) {
        // Bo qua neu file khong ton tai (lan dau chay)
    }
...
// Ap dung try-catch tu QuanLyChiTieu.cpp de chong crash khi file hong
// ===================================================================
...
// Luu tat ca file trong 1 lan goi (tien dung sau moi thao tac)
void saveAll() {
    saveTransactions();
    saveBudgets();
    saveCategories();
...
// ===================================================================
// MODULE 7: LOGIC NGHIEP VU NGAN SACH (BUDGET BUSINESS LOGIC)
// ===================================================================
...
// Kiem tra xem giao dich moi co vuot budget khong (dung khi nhap lieu)
...
// Sinh ID moi cho giao dich (max ID hien co + 1, an toan sau khi xoa)
int getNextTransID() {
    int mx = 0;
    for (int i = 0; i < gTransactions.size(); i++)
...
void addTransaction() {
    ...
    // Canh bao vuot budget REAL-TIME (lay tu QuanLyChiTieu.cpp)
    if (t.type == 0) {
        long long limit = 0;
...
// 2. Xem danh sach giao dich (co loc)
...
// 8. Tim kiem giao dich (case-insensitive)
...
// 4b. Thong ke theo nam (co bang tung thang)
void calcByYear() {
    int year = inputInt("  Nhap nam: ");
    long long thu = 0, chi = 0;
...
void calcByCategory() {
    ...
    // Vi khong co <map>, dung mang phu (chi so la catID)
...
// 7. Bieu do chi tieu text (lay tu ducanh.cpp, cai tien phan tram)
void printChart() {
    int month = inputInt("  Nhap thang (1-12): ");
    int year  = inputInt("  Nhap nam         : ");
...
// ===================================================================
// MODULE 10: QUAN LY NGAN SACH (BUDGET)
// ===================================================================
...
// Dat hoac cap nhat ngan sach (update_or_insert logic tu QuanLyChiTieu.cpp)
...
int main() {
    ...
    while (choice != 0) {
        ...
        if (!(cin >> choice)) {
            cin.clear();
            string tmp; getline(cin, tmp); // hut sach ky tu loi
            choice = -1;
